...
def simple_footprint(ratios={'u': 1, 'g': 1, 'r': 1, 'i': 1, 'z': 1, 'y': 1}, nside=32):
    from lsst.sims.utils import hpid2RaDec
    result = {}
    fp = np.zeros(hp.nside2npix(nside))
    ra, dec = hpid2RaDec(nside, np.arange(fp.size))
    wfd_dec_min = -62.5
    wfd_dec_max = 3.6
    good = np.where((dec >= wfd_dec_min) & (dec <= wfd_dec_max))[0]
    fp[good] = 1
    for filtername in 'ugrizy':
        result[filtername] = fp*ratios[filtername]
    return result
